...
trait BookHydrationMapping
{
    private function bookHydrator(SQLite3 $database) : Hydrates
    {
        return MappedHydrator::fromThis(Mapping::ofThe(Book::class,
            HasOneEmbedded::inProperty('title', $this->titleHydrator()),
            HasOneEmbedded::inProperty('isbn', $this->isbnHydrator()),
            HasOneEmbedded::inProperty('author', $this->authorHydrator()),
            ...
            HasManyProxies::inPropertyWithDifferentKey('contents', 'chapters',
                ...
                ProxyFactory::fromThis(
                    ...
                    $this->chapterLoaderFactory($database),
                    $this->updaterFactory()
                )
            )
        ));
    ...
    private function titleHydrator() : Hydrates
    {
    ...
    private function isbnHydrator() : Hydrates
    {
    ...
    private function authorHydrator() : Hydrates
    {
        return MappedHydrator::fromThis(Mapping::ofThe(Author::class,
            StringValue::inPropertyWithDifferentKey('firstName',
                'author_first_name'
            ),
            StringValue::inPropertyWithDifferentKey('lastName',
                'author_last_name'
            )
        ));
    }

    private function chapterLoaderFactory(SQLite3 $database) : ProducesProxyLoaders
    {
        return new ChapterLoaderFactory(
            $database,
    ...
    {
        return new ContentsUpdaterFactory;
    }
}
